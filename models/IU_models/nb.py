...
def full_df_nb(df, config, y_str, experiment_name):
    acc_list = []
    prec_list = []
    rec_list = [] 
    bal_acc_list = []
    ...
    model, acc, prec, rec, bal_acc, f1, auc_roc = nb_complete(config, train_df, test_df, y_str, 0, experiment_name)
    acc_list.append(acc)
    prec_list.append(prec)
    rec_list.append(rec)
    bal_acc_list.append(bal_acc)
