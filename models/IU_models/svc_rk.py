...
def SVC_train(config, X_train, y_train, X_test, y_test):
    #fix all experiment files with grid_search once you get the time 
...
def shap_viz(model, X_train, y_str):
    ...
    plt.savefig(fr'results/SVCRK figures/SHAP/{y_str}.png')
    plt.close()
...
def svc_complete(config, train_df, test_df, y_str):
    if config.getboolean('SVCRK', 'Classifier'):
        ...
        print(f'SVC RADIAL: Acc: {accuracy}, Prec: {precision}, Rec: {recall}, BALANCED ACC: {balanced_acc}, F1: {f1} , AUC ROC: {auc_roc}')
        ...
        # plt.savefig(fr'results/SVCRK figures/accuracy vis/{y_str}_accuracy_visual.png')
        # plt.close()
...
def kfolds_svc_rk(k, config, y_str):
    acc_list = []
    prec_list = []
    rec_list = [] 
    bal_acc_list = []
...
def full_df_svcrk(df, config, y_str):
    acc_list = []
    prec_list = []
    rec_list = [] 
    bal_acc_list = []
